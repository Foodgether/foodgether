...
const OrderInfo = ({ inviteId, orderList, menu, orderStatus }: OrderInfoProps) => {
  const setOrder = useSetAtom(orderAtom)

  const handleConfirmOrder = async () => {
    const rawConfirmOrderResponse = await fetch(`${BACKEND_URL}/order/${inviteId}/confirm`, {
      ...fetchConfigs,
      method: 'POST',
    })
  ...
  const handleCancelOrder = async () => {
    const rawCancelOrderResponse = await fetch(`${BACKEND_URL}/order/${inviteId}/cancel`, {
      ...fetchConfigs,
      method: 'POST',
    })
